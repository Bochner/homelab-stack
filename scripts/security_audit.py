                ...
                for pattern in secret_patterns:
                    if pattern.lower() in env_key.lower():
                        if isinstance(env_value, str) and not env_value.startswith(
                            ...
                            if len(env_value) > 0 and not env_value.lower() in [
                                "changeme",
                                "your-password",
                                "example",
                            ]:
    ...
    def print_report(self, audit_result: Dict):
        ...
        if audit_result["findings"]:
            ...
            for severity in ["HIGH", "MEDIUM", "LOW", "INFO", "ERROR"]:
                severity_findings = [
                    f for f in audit_result["findings"] if f["severity"] == severity
                ]
...
def main():
    ...
    for finding in results["findings"]:
        if finding["severity"] == "HIGH":
            ...
            if any(
                pattern in finding["message"].lower()
                for pattern in ["docker.sock", "host network", "port", "privileged"]
            ):
    ...
    if critical_issues:
        ...
        if high_count > 0:
            print(
                "ℹ️  Note: High severity findings are typically acceptable in homelab environments"
            )
            print(
                "   (Docker socket access, port exposure, privileged containers for functionality)"
            )
